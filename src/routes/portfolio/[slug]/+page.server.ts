...
export const load: PageLoad = async ({ params, fetch }) => {
    const url = 'https://api.valleywidebuilders.com/api/portfolios?filters[slug][$eq]='+params.slug+'&populate=deep,3';
    const headers = {
        Authorization: 'Bearer ' + STRAPI_API 
    }    
    const response = await axios.get(url, { headers });

    const urlporfolios = 'https://api.valleywidebuilders.com/api/portfolios?fields[0]=title&fields[1]=slug&populate[2]=featuredImage';
    ...
    // const rpUrl = 'https://api.valleywidebuilders.com/api/portfolios?filters[categories][id][$eq]='+randomCat+'&populate=deep,2';
