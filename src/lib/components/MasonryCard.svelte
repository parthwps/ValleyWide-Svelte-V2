...
async function fetchPortfolios(){
    const url = "https://vwapi.netdevs.net/api/portfolios?filters[categories][id][$eq]="+id+"&populate=deep,2";
    const headers = {
        ...
        {#if index < propCount}
            <div class="masonry-items" in:fly="{{ y: 0, duration: 1000, delay:index * 1500}}" out:fly="{{y:0, duration:1000 }}"> 
                <a data-sveltekit-reload href="/portfolio/{project.attributes.slug}" class="zoomImg">  
                    ...
                    <ImageLoader src="https://vwapi.netdevs.net/{project.attributes.featuredImage.data.attributes.url}" lowRes="https://vwapi.netdevs.net/{project.attributes.featuredImage.data.attributes.formats.small.url}" alt="{project.attributes.title}"></ImageLoader>
                    <!-- <img loading="lazy" src="https://vwapi.netdevs.net/{project.attributes.featuredImage.data.attributes.url}" alt="{project.attributes.title}" >    -->
                    {:else}
